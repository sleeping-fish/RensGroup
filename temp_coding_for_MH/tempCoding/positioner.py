...
class Matrix:
    ...
    def ProcessMartix(MatrixSet, WaveNumber, Label, Z, T_Set):
        ...
        WaveSet = []
        # 这边用字典存了T_set，不够可以加
        label_T_dict = {'1': T_Set[0], '2': T_Set[1], '3': T_Set[2], '4': T_Set[3], '5': T_Set[4]}
    ...
    def SaveData(DataSet, WaveNumber, Width):
        ...
        Width = np.array(Width).reshape(-1, 1)[:DataSet.shape[0]]
    ...
    def getResidue(self, SaveName):
        ...
        for aa in self.aaList:
            ...
            if aa.get_resname() == "HIS":  # 如果残基为组氨酸，则采一部分原子，需要5个向量
                ...
                vector_1 = (Atom5 - Atom1)[0:3]  # c-n,暂时没用, [-0.97, -1.083, -0,131]
                vector_3 = (Atom4 - Atom1)[0:3]  # c-c,用于ch2, [1.175, -0.224, -0.996]
                vector_4 = (Atom4 - Atom3)[0:3]  # 咪唑环向量1, [-1.344, -0.488, -0.436]
                vector_5 = (Atom3 - Atom2)[0:3]  # 咪唑环向量2, [-1.159, 0.684, -0.284]
                phStd1 = np.array([-1.344, -0.488, -0.436])
                phStd2 = np.array([-1.159, 0.684, -0.284])
                CH2td1 = np.array([1.175, -0.224, -0.996])
                # 苯环T记为1
                T1 = RM.Calculate(phStd1, vector_4, phStd2, vector_5)
                # c-n
                CN_Std = np.array([-0.97, -1.083, -0, 131])
                # T3 = RM.Calculate(CN_Std, vector_1, CH2td1, vector_3)
                ...
                MatrixSet, Label, WaveNumber, Width = Matrix.LoadMartix(
                    FilePath=self.MatrixDir + fr'\HIS.xlsx')
            ...
            if aa.get_resname() == "PHE":
                ...
                vector_2 = (Atom5 - Atom1)[0:3]  # c-n,暂时没用, [-1.137, -0.799, -0.45]
            ...
            if aa.get_resname() == "TRP":
                ...
                CH2td1 = np.array([0.88, 0.472, -1.189])
                # 苯环T记为1
                T1 = RM.Calculate(phStd1, vector_4, phStd2, vector_5)
                # c - c(o)
            ...
            TempSet = np.concatenate((Data_TRP,
                                      Data_HIS,
                                      Data_ASP,
                                      Data_ALA,
                                      Data_GLU,
